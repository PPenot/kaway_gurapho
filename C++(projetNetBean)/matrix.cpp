...
{
	if(rows==0)
		cout << "Matrix with no row";
	else if(cols==0)
		cout << "Matrix with no column";
	else
	{
		for(int i=0;i<rows;i++)
		{
...
void Matrix::subRows (int r,const float val)
{
    ...
    for(int i=0 ;i<cols;i++)
    {
        if(values[r][i]!=HUGE_VAL)
            values[r][i] =values[r][i] - val;
    }
}
...
void Matrix::subCols (int c,const float val)
{
    ...
    for(int i=0 ;i<rows;i++)
    {
        if(values[i][c]!=HUGE_VAL)
            values[i][c] =values[i][c] - val;
    }
}
